...
def calcHelicityLep( lep, met, btag ):
    mw = 80.385

    #making 3 vectors
    plep = rt.TVector3(lep.px() , lep.py()   , lep.pz() ) 
    pnu  = rt.TVector3(met.px() , met.py()   , 0.       ) 
    pb   = rt.TVector3(btag.px(), btag.py()  , btag.pz()) 
    
    #compute pnuz
    b = - 2.* (mw*mw / 2. + plep.Dot(pnu) ) * lep.pz()
    a = lep.px() * lep.px() + lep.py() * lep.py()
    c = - ( mw*mw / 2. + plep.Dot(pnu) ) * ( mw*mw / 2. + plep.Dot(pnu) ) + plep.Mag2()*pnu.Mag2()
    delta = b*b - 4.*a*c
    if delta < 0. :
        print 'neg delta', delta
        return -1.
    pnuz = 0.5*( -b + math.sqrt(max(0., delta)) )
    pnu.SetZ(pnuz)
    
    #compute boost to W rest frame
    betaW  = (plep+pnu)
    betaW *= 1./(lep.energy()+pnu.Mag())
    gammaW = 1 / math.sqrt( 1 - betaW.Dot(betaW))
    
    #compute lep momentum in W rest frame
    plstar =  - betaW
    plstar *= lep.energy()
    plstar += plep
    plstar *= gammaW

    #compute boost to top rest frame
    betaTop  = (plep+pnu+pb)
    betaTop *= 1./ (lep.energy()+pnu.Mag()+btag.energy())
    gammaTop = 1 / math.sqrt( 1 - betaTop.Dot(betaTop))

    #compure W momentum in top rest frame
    pwstarTop  = -betaTop
    pwstarTop *= (lep.energy()+pnu.Mag())
    pwstarTop += plep
    pwstarTop += pnu
    pwstarTop *= gammaTop

    #helicity angle
    cosThetaStar = pwstarTop.Dot(plstar) / (pwstarTop.Mag() * plstar.Mag() )
